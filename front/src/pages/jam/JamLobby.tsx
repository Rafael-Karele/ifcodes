  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center p-8">
      <div className="w-full max-w-lg rounded-xl border border-gray-200 bg-white p-8 text-center shadow-lg">
        ...
        <h2 className="mb-2 text-2xl font-bold text-gray-800">{session.titulo}</h2>
        <p className="mb-1 text-gray-500">
          Problema: {session.problema?.titulo || "Carregando..."}
        </p>
        {session.instrucoes && (
          <p className="mb-4 text-sm text-gray-500">{session.instrucoes}</p>
        )}
        ...
        <div className="mb-6 rounded-lg bg-gray-50 p-4">
          ...
          {participants.length === 0 && (
            <p className="text-sm text-gray-400">Aguardando participantes...</p>
          )}
        </div>
